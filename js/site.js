...
(function () {
    ...
    if ('IntersectionObserver' in window && !reduce) {
        var sel = '.section-label, .section-title, .section-intro, .step, .board-item, .board-value, .conversation-exchange, .testimonial, .pricing-card, .pilot-description, .about-block, .lead-prose, .compare, .pull-quote, .scope-item, .not-block, .value-maths, .pillar, .trap-list, .trap-close, .hero-content > *';
        var els = Array.prototype.slice.call(document.querySelectorAll(sel));
        ['.step', '.board-item', '.testimonial', '.pricing-card', '.scope-item', '.pillar'].forEach(function (s) {
